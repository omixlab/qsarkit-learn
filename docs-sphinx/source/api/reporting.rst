...
.. currentmodule:: qsarkit.reporting

QSAR reports in Markdown, HTML and JSON, QMRF-style OECD reporting, and
the Plotly figures that go in them.
...
The Williams plot is the standard OECD-facing diagnostic, putting
leverage against standardized residual so influential points and outliers
...
The same report object renders every way you might need it, and each
carries the tables:
...
Plain text for a terminal or an email; Markdown for a repository; HTML
for sharing; JSON for a downstream system; PDF for a submission:
...
A section can carry prose, a property table, a DataFrame and figures, and
all four survive into every format:
...
Nested values are flattened rather than printed as a Python repr, which
matters because a validation report routinely contains one:
...
:class:`OECDReportBuilder` structures a report around the five validation
principles:
...
the builder tracks them explicitly rather than letting an omission look
like an absence of problems.

API
---

.. automodule:: qsarkit.reporting
   :members:
   :show-inheritance:
...
- European Chemicals Agency (2016). "Practical Guide: How to Use and
  Report (Q)SARs." :doi:`10.2823/81818`
